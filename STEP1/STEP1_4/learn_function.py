...
def choose_trainset(FGL, i):
    ...
    y_train = pd.get_dummies(y_train_list[i].values)['PSR']
    X_test = X_test_list[i].values
    y_test = pd.get_dummies(y_test_list[i].values)['PSR']
...
def train_data_info(FGL, i):
    ...
    X_test = X_test_list[i].values
    # csvにデータの大きさの情報を保存
    path = "/Users/kawakami/Desktop/fight/STEP1/STEP1_4/result/DataSize{}.csv".format(i)
...
def learn(FGL, i):
    ...
    # csvに予測精度結果を保存
    path = "/Users/kawakami/Desktop/fight/STEP1/STEP1_4/result/model_acc_dataset{}.csv".format(i)
...
def gb_model(TssScore, X_test, X_train, cm_score, model, params, tss_FGL, y_test, y_train):
    ...
    clf = grb.best_estimator_.predict(X_test)
    tss_FGL.append(tss(y_test, clf))
